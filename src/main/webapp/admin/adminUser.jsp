...
<script type="text/javascript">
	...
	$(function() {
		...
		$(document).on("click", "main .card-body table > tbody td", function() {
			...
			$("span.name").text(tr.find("td:eq(2)").text());
			$("span.age").text(tr.find("td:eq(3)").text());
			...
			if ($("#answer").val() !== "") {
			    ...
			    $(document).on("click","#userState",function(event) {
			    	event.preventDefault();
			    	  event.stopPropagation();
			    	  
			    	 let isChecked = $(this).is(":checked");
			    	 localStorage.setItem("userStateChecked", isChecked);
			    	 checkBoxVisibility(isChecked);
			    ...
			    function checkBoxVisibility(isChecked) {
			    	  $("table#datatablesSimple tbody tr").each(function() {
			    	    let userState = $(this).find("td:nth-child(6)").text();
			    	    if (isChecked && userState === "N") {
			    	      $(this).show();
			    	    } else if (!isChecked) {
			    	      $(this).show(); // 체크박스가 체크되지 않은 경우 모든 회원 표시
			    	    } else {
			    	      $(this).hide();
			    	    }
			    	  });
				...
				<div class="container-fluid px-4">
					...
					<ol class="breadcrumb mb-4">
						<li class="breadcrumb-item"><a
							href="${path}/front?key=adminUser&methodName=selectAll">새로고침</a></li>
					...
					<div>
						...
						<form name="inForm" method="post" id="inForm"
							action="${path}/front?key=adminUser">
							<table>
								<tr>
									<th>UserSeq</th>
									<th>UserID</th>
								...
								<tr>
									<td><input type="text" size="7" name="userSeq"
										id="userSeq"></td>
					...
					<div class="card mb-4">
						<div class="card-header">
							<form id = "userForm" method="post" action="#">
								<i class="fas fa-table me-1"></i> 회원정보조회
								<input class="form-check-input" id="userState"  name="userState" type="checkbox" value="" />
	                            <label class="form-check-label" for="userState">Y/N</label>
